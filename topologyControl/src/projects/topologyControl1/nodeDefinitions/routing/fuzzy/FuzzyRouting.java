...
import projects.topologyControl1.nodes.messages.BeaconMessage;
import projects.topologyControl1.nodes.messages.EventMessage;
...
import projects.topologyControl1.nodes.timers.SimpleMessageTimer;
...
public class FuzzyRouting implements IRouting {
		...
		if (floodMsg.immediateDestination != null){
			if (floodMsg.immediateDestination.equals(node)){ //It is a message sent by the Base Station to its neighbors
				...
				if (re == null){
					...
					fuzzyRoutingTable.put(floodMsg.baseStation, new FuzzyRoutingEntry(Integer.valueOf(floodMsg.sequenceID), Integer.valueOf(floodMsg.hopsToBaseStation), (Node)floodMsg.forwardingNode, Boolean.TRUE, floodMsg.index));//TINHA FSL AQUI
			...
			if (floodMsg.forwardingNode.equals(node)){ // The message bounced back. The node must discard the msg.
				...
				if (re == null){
					//fsl = Utils.calculateFsl(energy, numHops);
					fuzzyRoutingTable.put(floodMsg.baseStation, new FuzzyRoutingEntry(Integer.valueOf(floodMsg.sequenceID), Integer.valueOf(floodMsg.hopsToBaseStation), (Node)floodMsg.forwardingNode, Boolean.TRUE, floodMsg.index));
					//myLog.logln("Rota com Indice"+floodMsg.index + ";" + Tools.getGlobalTime()+";Rota Adicionada;"+node.ID+";"+floodMsg.baseStation+";"+floodMsg.forwardingNode.ID+";"+floodMsg.sequenceID+";"+floodMsg.hopsToBaseStation+";"+floodMsg.energy+";"+fsl); 
				}else{
					...
					if (field.getSequenceNumber() < floodMsg.sequenceID) { //Update an existing entrie						
						...
						//fsl = Utils.calculateFsl(energy, numHops);
						re.addField(Integer.valueOf(floodMsg.sequenceID), Integer.valueOf(floodMsg.hopsToBaseStation), (BasicNode)floodMsg.forwardingNode, Boolean.TRUE, floodMsg.index);
						//myLog.logln("Rota com Indice"+floodMsg.index + ";" +Tools.getGlobalTime()+";Rota Adicionada;"+node.ID+";"+floodMsg.baseStation+";"+floodMsg.forwardingNode.ID+";"+floodMsg.sequenceID+";"+floodMsg.hopsToBaseStation+";"+floodMsg.energy+";"+fsl);
					}else{
						forward = Boolean.FALSE;
					}
	...
	private void receiveFloodFindMessage(Message message){
		...
		if (floodMsg.forwardingNode.equals(node)){ // The message bounced back. The node must discard the msg.
			...
			if (re == null){
				fuzzyRoutingTable.put(floodMsg.baseStation, new FuzzyRoutingEntry(Integer.valueOf(floodMsg.sequenceID), Integer.valueOf(floodMsg.hopsToBaseStation), (Node)floodMsg.forwardingNode, Boolean.TRUE, 0));
				...
				if (re.getFieldsSize() < numBuffer) { 
					re.addField(Integer.valueOf(floodMsg.sequenceID), Integer.valueOf(floodMsg.hopsToBaseStation), (BasicNode)floodMsg.forwardingNode, Boolean.TRUE, 0);
					//myLog.logln("Rota;"+Tools.getGlobalTime()+";Rota Adicionada;"+node.ID+";"+floodMsg.baseStation+";"+floodMsg.forwardingNode.ID+";"+floodMsg.sequenceID+";"+floodMsg.hopsToBaseStation+";"+floodMsg.energy+";"+fsl);
				}else{
